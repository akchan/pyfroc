...
@dataclass
class SegNRRD:
    space_directions: np.ndarray  # (3x3): (voxel_dir, xyz)
...
class SegNRRDLoader(BaseLoader):
    ...
    def read_responses(cls, case_dir_path: str) -> list[Response]:
        ...
        for segnrrd_path in cls._list_segnrrd_path(case_dir_path):
            segnrrd = cls._read_segnrrd(segnrrd_path)
            series_responses.extend(cls._segnrrd2responses(segnrrd))

        return series_responses

    @classmethod
    def _segnrrd2responses(cls, segnrrd: SegNRRD, mask_dtype=np.int8) -> list[Response]:
    ...
    @classmethod
    def _mask2minsphere(cls, mask: np.ndarray,
                        space_directions: np.ndarray,
                        mask_dtype=np.int32) -> tuple[np.ndarray, float]:
        # Consider only the edge points to reduce the computational cost
        mask = (mask > 0).astype(mask_dtype)

        assert mask.max() > 0, "mask should have at least one positive value"
    ...
    @classmethod
    def _idx2coords(cls, idx, space_directions, origin=np.zeros(3)):
        return np.dot(idx, space_directions) + origin

    @classmethod
    def _list_segnrrd_path(cls, dir_path) -> list[str]:
        return glob.glob(os.path.join(dir_path, "*.seg.nrrd"))

    @classmethod
    def _read_segnrrd(cls, segnrrd_path) -> SegNRRD:
        vol, header = nrrd.read(segnrrd_path)

        if vol.ndim == 3:
            vol = np.expand_dims(vol, axis=0)

        parsed_header = cls._parse_seg_nrrd_header(header)
    ...
    @classmethod
    def _parse_confidence_from_seg_name(cls, name: str) -> int:
    ...
    @classmethod
    def _parse_seg_nrrd_header(cls, header: dict) -> dict:
        ret = {
        ...
        for key in header.keys():
            ...
            if key.endswith("LabelValue"):
                ...
                seg_dict[id].confidence = cls._parse_confidence_from_seg_name(name)
