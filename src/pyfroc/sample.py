...
def download_dicom_from_nbia(se_uid_list, target_path="./sample_data/dicom"):
    os.makedirs(target_path, exist_ok=True)

    nbia.downloadSeries(se_uid_list, input_type="list", path=target_path)


def download_experiment_data(url, target_dir):
...
def download_sample_data(target_dir="./sample_data"):
    ...
    download_experiment_data(release_url, target_dir)

    # Download dicom files from NBIA
    dicom_path = os.path.join(target_dir, "dicom")
    download_dicom_from_nbia(se_uid_list, target_path=dicom_path)
